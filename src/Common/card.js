...
const cardImage = ({ data, startDate, endDate, navigation }) => {
  const room = data;

  goDetail = () => {
    navigation.navigate('staythird', { 'roomId': room.roomId, 'hotelId': room.hotel.hotelId,'startDate': startDate, 'endDate': endDate, navigation });
  }
  ...
  return (
    <View style={{ zIndex: 1 }}>
      <Card>
        ...
        <Text style={{ fontWeight: 'bold', fontSize: 18, color: '#5f5b5b', margin: 10 }}>{room.hotel.name}</Text>
